...
@Component({
  selector: 'app-user-education-block',
  templateUrl: './user-education-block.component.html',
  styleUrls: ['./user-education-block.component.css']
})
export class UserEducationBlockComponent implements OnInit {
  @Input() TagObj:any;
  ...
  removeExtras = (obj:any,extras:string[])=>{
    ...
    event.stopPropagation();
        ...
        if(newObj['_id'] == undefined || newObj['_id'] == null){
          ...
          this.http.addNewUserEducation(newObj,token).subscribe((data)=>{
            if(data.Status == 200){
              console.log(data);
          ...
          var token = this.jwt.getToken() ?? "";
          this.http.editUserEducation(newObj,token).subscribe((data)=>{
            if(data.Status == 200){
              this.TagObj = data.Data;
              this.TagObjChange.emit(this.TagObj);
              console.log(data);
    ...
    deleteTag = (event:any)=>{
      if(this.TagObj['_id'] == undefined || this.TagObj['_id'] == null){
        ...
        var token = this.jwt.getToken() ?? "";
        this.http.deleteUserEducation(this.TagObj,token).subscribe((data)=>{
          if(data.Status == 200){
            this.TagObjDelete.emit(true);
            alert("success");
          }
